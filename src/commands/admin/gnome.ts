...
export default new Command({
    ...
    execute: async (client, interaction) => {
        ...
        if (!channel || !channel.isVoiceBased)
            return await replyMessage(interaction, 'Please provide a valid voice channel.', true);
        if (interaction.user.id !== process.env.LAMAR_ID)
            return await replyMessage(interaction, 'https://www.youtube.com/watch?v=6n3pFFPSlW4', true);
